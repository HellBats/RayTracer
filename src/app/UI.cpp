...
void UI::RenderPanels(GLuint texture,int window_width, int window_height, int view_width, int view_height
    ...
    ImGui::BeginChild("Camera",ImVec2(300,250));
        ...
        ImGui::EndChild();
    ImGui::EndChild();
    ImGui::BeginChild("Lights",ImVec2(300,300));
        
            ImGui::SliderFloat("LightIntensity",&(scene.lights[0].intensity),0,2000);
            if(scene.lights[0].type==LightType::DISTANT)
            {
                ImGui::SliderFloat("directionx",&(scene.lights[0].distant_light.direction.x),-200,60);
                ImGui::SliderFloat("directiony",&(scene.lights[0].distant_light.direction.y),-200,60);
                ImGui::SliderFloat("directionz",&(scene.lights[0].distant_light.direction.z),-200,60);
            }
            else
            {
                ImGui::SliderFloat("positionx",&(scene.lights[0].point_light.position.x),-200,60);
                ImGui::SliderFloat("positiony",&(scene.lights[0].point_light.position.y),-200,60);
